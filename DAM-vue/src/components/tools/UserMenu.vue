...
<!--        </a-menu-item>-->
<!--        <a-menu-item key="1">-->
<!--          <router-link :to="{ path: '/account/settings' }">-->
<!--            <a-icon type="setting" />-->
<!--            <span>账户设置</span>-->
<!--          </router-link>-->
<!--        </a-menu-item>-->
...
<!--        </a-menu-item>-->
    ...
    <span class="action" v-if="showLogout">
...
export default {
  ...
  computed: {
    ...
    showLogout(){
      ...
      const allowLogoutUsers = ['admin', 'secaudit', 'secadmin', 'sysadmin'];
      return allowLogoutUsers.includes(username);
    }
  },
  /* update_end author:zhaoxin date:20191129 for: 做头部菜单栏导航*/
  watch: {
      ...
      this.$confirm({
        ...
        onOk() {
          return that.Logout({}).then(() => {
            ...
            that.$router.push({ path: '/user/login?manual=1' });
            //window.location.reload()
            // todo: 退出登录避免 key 登录情况下退出了又自动登录了
            // window.close() 由于浏览器安全策略设置并不能关闭不是 window.open() 打开的页面
