<template>
  <div :id="containerId" style="position: relative">
    ...
    <!-------------------------- 密级选择 ----------------------------->
    <div>
      <!--      <j-dict-select-tag type="list" v-model="fileSecretLevel" :dictCode="system_security" placeholder="请选择文件密级"-->
      <!--                         style="float: left; margin:4px 4px; width:35%; padding: 0;"/>-->
      <!--                             :minLevel="maxFileLevel"-->
      <j-secret-level-select v-if="!disabled" style="float: left; margin:4px 4px; width:35%; padding: 0;" type="list"
                             v-model="fileSecretLevel" :maxLevel="maxLevel" placeholder="请选择文件密级"
                             :key="timer+maxLevel"
                             :showKOption="showKOption" />
      ...
      <a-upload
        ...
        :class="{'uploadty-disabled':disabled,'float':'left'}">
        <template>
...
import { getFileAccessHttpUrlJszt } from '@/api/manage'
...
export default {
  name: 'JUploadSecretJszt',
  components: { JSecretLevelSelect },
  data() {
    return {
      timer: '',
      maxFileLevel: 1,
      system_security: 'system_security_gk',
      headers: {},
      fileList: [],
      newFileList: [],
      uploadGoOn: true,
  ...
  watch: {
    ...
    //最大密级改变后，需判断是否需要修改fileSecretLevel
    maxLevel: {
      immediate: true,
      handler(value) {
        if (value) {
          if (Number(value) < Number(this.fileSecretLevel)) {
            this.fileSecretLevel = value
          }
        }
  ...
  computed: {
    ...
    uploadAction() {
      return window._CONFIG['domianURL'] + '/sys/common/uploadJszt?fileSecretLevel=' + this.fileSecretLevel
    }
  },
  created() {
    ...
    //---------------------------- end 图片左右换位置 -------------------------------------
    EventBus.$on('cleanLevel', this.cleanLevel)
    setTimeout(() => {
      this.getMaxFileLevel()
    }, 500)
    // console.log('this.fileList: ', this.fileList)
  },
  beforeDestroy() {
    EventBus.$off('cleanLevel')
  },
  methods: {
    //查看当前附件最大密级并设置fileSecretLevel
    getMaxFileLevel() {
      this.maxFileLevel = 1
      setTimeout(() => {
        for (let i = 0; i < this.fileList.length; i++) {
          var mj = this.fileList[i].name.slice(0, 2)
          if (mj === '公开') {
            this.maxFileLevel = Math.max(this.maxFileLevel, 1)
          } else if (mj === '内部') {
            this.maxFileLevel = Math.max(this.maxFileLevel, 2)
          } else if (mj === '秘密') {
            this.maxFileLevel = Math.max(this.maxFileLevel, 3)
          } else if (mj === '机密') {
            this.maxFileLevel = Math.max(this.maxFileLevel, 4)
          }
        }
        // 将fileSecretLevel设置为最大密级，但不超过maxLevel限制
        // if (this.maxFileLevel > this.fileSecretLevel) {
        //   this.fileSecretLevel = this.maxLevel && this.maxLevel < this.maxFileLevel ? this.maxLevel : this.maxFileLevel
        // }
      })
      this.timer = new Date().getTime()
    },

    //清除当前文件密级,e应该大于等于文件密级
    cleanLevel(e) {
      if (Number(e) < Number(this.fileSecretLevel)) {
        this.fileSecretLevel = e
      }
    },
    initFileListArr(val) {
      if (!val || val.length === 0) {
      ...
      for (var a = 0; a < val.length; a++) {
        let url = getFileAccessHttpUrlJszt(val[a].filePath)
        fileList.push({
          uid: uidGenerator(),
          name: val[a].fileName,
          status: 'done',
    ...
    initFileList(paths) {
      ...
      for (var a = 0; a < arr.length; a++) {
        let url = getFileAccessHttpUrlJszt(arr[a])
        fileList.push({
          uid: uidGenerator(),
        ...
        if (info.file.response.success) {
          fileList = fileList.map((file) => {
            if (file.response) {
              ...
              file.url = getFileAccessHttpUrlJszt(reUrl)
              if(reUrl){
                file.name = reUrl.split('/').pop();
              }
            }
            return file
          })
