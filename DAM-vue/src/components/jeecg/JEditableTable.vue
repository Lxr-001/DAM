  ...
  export default {
    ...
    watch: {
      ...
      maxLevel: {
        immediate: true,
        handler(newVal) {
          const securityColumn = this.columns.find(column => column.dictCode === "system_security");
          //重新加载该列
          this._loadDictConcatToOptions(securityColumn);
        }
      },
    ...
    methods: {
      ...
      _loadDictConcatToOptions(column) {
        initDictOptions(column.dictCode).then((res) => {
          if (res.success) {
            ...
            if(column.dictCode === 'system_security'){
                newOptions = newOptions.filter(e=>e.value <= Number(this.maxLevel))
            }
            this.$set(column, 'options', newOptions)
            var currScurity = this.selectValues[this.secretLevelId];
            if(currScurity && currScurity > this.maxLevel){
              this.selectValues[this.secretLevelId] = this.maxLevel;
            }
          } else {
      ...
      buildTooltipProps(row, col, id) {
        if(col.dictCode && col.dictCode === "system_security"){
          this.secretLevelId = id;
        }
