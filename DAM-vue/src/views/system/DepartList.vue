<template xmlns:background-color="http://www.w3.org/1999/xhtml">
  <a-row :gutter="10" class="depart-layout-row">
    ...
    <a-col :sm="24" class="depart-right-col">
      <a-tabs :activeKey="activeTabKey" class="depart-right-tabs" @change="handleTabChange">
        <a-tab-pane tab="基本信息" key="1" class="depart-tab-pane">
          <a-card :bordered="false" v-if="selectedKeys.length>0" class="depart-side-card">
            <a-form-model ref="form" :model="model" :rules="validatorRules">
              ...
              <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="orgCode" label="机构编码" >
                <a-input placeholder="请输入机构编码" v-model="model.orgCode" />
              </a-form-model-item>
          ...
          <a-card v-else class="depart-side-card">
            <a-empty>
              <span slot="description"> 请先选择一个部门! </span>
            </a-empty>
          </a-card>
        </a-tab-pane>
        <a-tab-pane tab="用户信息" key="2" class="depart-tab-pane" forceRender>
          <div class="depart-user-info-wrap">
            <user-list ref="userList" :filterByOrgUnit="true" :orgUnit="userListOrgUnit" />
          </div>
        </a-tab-pane>
        <!--        <a-tab-pane tab="成品权限" key="2" forceRender>-->
        <!--          <pbs-auth-modal ref="departAuth" />-->
        <!--        </a-tab-pane>-->
        <!--        <a-tab-pane tab="同厂同类权限" key="3" forceRender>-->
        <!--          <tctl-auth-modal ref="tctlAuth" />-->
        <!--        </a-tab-pane>-->
      </a-tabs>
...
<script>
  ...
  import Vue from 'vue'
  import TctlAuthModal from '@views/system/modules/TctlAuthModal.vue'
  import PbsAuthModal from '@views/system/modules/PbsAuthModal.vue'
  // 表头
  const columns = [
    {
      title: '机构名称',
  ...
  export default {
    ...
    methods: {
      ...
      onSelect(selectedKeys, e) {
        ...
        const currentOrgUnit = (e && e.node && e.node.eventKey) || (selectedKeys && selectedKeys.length > 0 ? selectedKeys[0] : '') || record.id || record.key
        this.currSelected = Object.assign({}, record)
        this.model = this.currSelected
        this.selectedKeys = [record.key]
        this.model.parentId = record.parentId
        // 选择部门后，用户信息 Tab 展示该部门的用户
        this.userListOrgUnit = currentOrgUnit
        this.activeTabKey = '2'
        this.setValuesToForm(record)
        this.$refs.departAuth.show(record.id)
        this.$refs.tctlAuth.show(record.id)
      ...
      onClearSelected() {
        ...
        this.currSelected = {}
        this.selectedKeys = []
        this.userListOrgUnit = ''
        this.activeTabKey = '1'
        this.$refs.departAuth.departId = ''
        this.$refs.tctlAuth.departId = ''
      },
...
<style scoped>
  ...
  .depart-user-info-wrap {
    height: 100%;
    overflow-y: auto;
  }

  ::v-deep .depart-right-tabs .ant-tabs-content {
    background: #fff;
  }

  ::v-deep .depart-right-tabs .ant-tabs-tabpane {
    background: #fff;
  }
