...
<script>
import { queryPBSTreeList2, saveDepartPermission,queryDepartPermission } from '@/api/api'
      ...
      if (this.departId) {
        ...
        // 先调用你原有的 loadData 方法加载权限树
          // loadData 成功后，再加载已勾选的权限
            queryDepartPermission({ departId: this.departId , type: 0}).then((checkedRes) => {
  ...
  if (Array.isArray(node)) {
    node.forEach(item => {
      if (item.children && item.children.length > 0) {
        this.expandedKeysss.push(item.id); // 使用 id
        this.setThisExpandedKeys(item.children);
      }
    });
  } else if (node && node.children && node.children.length > 0) {
    this.expandedKeysss.push(node.id); // 使用 id
    this.setThisExpandedKeys(node.children);
  }
},
  getAllKeys(node) {
    if (Array.isArray(node)) {
