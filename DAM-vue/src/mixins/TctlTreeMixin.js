...
export const TctlTreeMixin = {
  ...
  methods: {
    ...
    treeModalOk() {
      ...
      this.loadTctlTree()
      //右侧列表刷新
      this.loadRightData(this.record.id, this.showSubNodesInfo)
    },
    loadTctlTree() {
      var that = this
      that.treeData = []
      that.catalogTree = []
      queryTctlNewTreeList({ showModel: false }).then((res) => {
        if (res.success) {
          //产品目录全选后，再添加产品目录，选中数量增多
          this.allTreeKeys = []
          for (let i = 0; i < res.result.length; i++) {
    ...
    onSelect(selectedKeys, e) {
      ...
      switch (e.node.dataRef) {
        case 1:
          this.record.kptqk_text = '同平台'
          break
        case 2:
          this.record.kptqk_text = '同主机'
          break
        case 3:
          this.record.kptqk_text = '跨主机'
          break
      }
      this.isLeaf = this.record.isLeaf
      console.log('叶子 tctlTreeMixin', this.isLeaf)
      // record.id 需要作为查询条件
      this.nodeId = this.record.id
      ...
      if (this.showSubNodesInfo || this.level === 0) {
        this.loadRightData(this.record.id, true)
      } else {
        this.loadRightData(this.record.id, false)
      }
      // this.scoreVisible = this.level !== 0
      // console.log('this.scoreVisible: ', this.scoreVisible)
      this.currSelected = Object.assign({}, this.record)
      this.model = this.currSelected
      this.selectedKeys = [this.record.key]
      this.model.bid = this.record.bid
