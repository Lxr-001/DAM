...
export const JeecgListNoLoadDataMixin = {
    data() {
        return {
    ...
    created() {
        if (!this.disableMixinCreated) {
            console.log(' -- mixin created without loading data first -- ');
            // this.loadData();
            // 初始化字典配置 在自己页面定义
            this.initDictConfig();
        }
    ...
    methods: {
        ...
        onSelectChange(selectedRowKeys, selectionRows) {
            ...
            let SimilarFactorData = [];
            SimilarFactorData.push(this.selectionRows);
            this.$emit('SimilarFactorData',SimilarFactorData);
        },
        onClearSelected() {
        ...
        codeBase64Encode: function (url) {
            var rv;
            rv = encodeURIComponent(url)
            rv = unescape(rv)
            rv = window.btoa(rv)
            return rv;
        },
