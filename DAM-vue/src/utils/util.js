...
export function validateNotJustWhitespace(rule, value, callback) {
  if (value) {
    const patt = /^[\s]*$/
    let pvalue = patt.test(value)
    if (pvalue) {
      callback(new Error('输入不能为空或仅包含空格'))
    } else {
      callback()
    }
  } else {
...
/**
 * 公用的时间比较验证器函数
 * @param otherFieldGetter
 * @param message
 */
export function validateTimeBefore(otherFieldGetter, message) {
  return function(rule, value, callback) {
    // otherFieldGetter 是一个函数，它应该返回要比较的另一个字段的值
    const otherValue = otherFieldGetter()
    if (value && otherValue && new Date(value) < new Date(otherValue)) {
