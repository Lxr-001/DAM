...
//	}
    ...
    public Result<?> getUserPermissionByToken(HttpServletRequest request) {
        ...
        try {
			...
			json.put("userSecurity", Math.min(systemSecurity, loginUser.getSecurity()));
    ...
    public Result<String> saveRolePermission(@RequestBody JSONObject json) {
        ...
        try {
            ...
            String errorMsg = this.sysRolePermissionService.saveRolePermission(roleId, permissionIds, lastPermissionIds);
...
//            List<String> permissions= sysPermissionService.listByIds(new ArrayList<>(Arrays.asList(permissionIds.split(",")))).stream().map(SysPermission::getName).collect(Collectors.toList());
            baseCommonService.addLog(
                    ...
                    8,  //用户/角色变更
                    "角色管理",
//                    "保存角色权限，角色名称：" + role.getRoleName()+"，权限："+String.join("、",permissions),  // 太长了
                    "保存角色权限，角色名称：" + role.getRoleName(),
                    "成功"
            );
    ...
    public Result<List<String>> queryDepartPermission(@RequestParam(name = "departId", required = true) String departId, @RequestParam(name = "type", required = true) Integer type) {
        Result<List<String>> result = new Result<>();
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        int secretLevel = Math.min(systemSecurity, user.getSecurity());
        try {
            LambdaQueryWrapper<SysDepartPermission> queryWrapper = new QueryWrapper<SysDepartPermission>()
                    .lambda()
                    .eq(SysDepartPermission::getDepartId, departId)
                    .eq(SysDepartPermission::getType, type);
            List<SysDepartPermission> list = sysDepartPermissionService.list(queryWrapper);
            ...
            baseCommonService.addLog(secretLevel, 2, 2, "部门授权信息", "查询部门授权信息", "成功");
            result.setSuccess(true);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
    ...
    public Result<String> saveDepartPermission(@RequestBody JSONObject json) {
        ...
        try {
            ...
            this.sysDepartPermissionService.saveDepartPermission(departId, permissionIds, lastPermissionIds, type);
            result.success("保存成功！");
            baseCommonService.addLog(secretLevel, 2, 2, "部门授权信息", "修改部门授权信息：修改部门为（" + sysDepart.getDepartName() + "） 详细ID：" + json, "成功");
     ...
     * @return
     */
    @RequestMapping(value = "/saveDepartPermission1", method = RequestMethod.POST)
    //@RequiresRoles({ "admin" })
    public Result<String> saveDepartPermission1(@RequestBody JSONObject json) {
        long start = System.currentTimeMillis();
        Result<String> result = new Result<>();
        try {
            String departId = json.getString("departId");
