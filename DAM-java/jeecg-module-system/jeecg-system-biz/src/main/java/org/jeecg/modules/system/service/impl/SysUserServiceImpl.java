...
import org.jeecg.modules.system.util.UserEditQueueConfig;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    @Autowired
    private UserEditQueueConfig queueConfig;
     ...
     */
    @Override
    public int getLogTypeByUserName(String username) {
        List<String> roleCodeList = this.getRole(username);
        ...
        if ("admin".equalsIgnoreCase(roleCode)) {
            return 0;   // 独立admin日志（调试用，正式版不存在超级管理员）
        }
        return 1;
    }
    ...
    public Result<?> resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) {
        ...
        this.userMapper.update(new SysUser().setPassword(password).setPasswdUpdateTime(new Date()), new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, user.getId()));
    ...
    public Result<?> changePassword(SysUser sysUser) {
        ...
        sysUser.setPasswdUpdateTime(new Date());
...
//			}
    ...
    @Override
    public Map<String, String> getOrgunitNamesByOrgunitIds(List<String> OrgunitIds) {
        String ids = "'" + String.join("','", OrgunitIds) + "'";
        QueryWrapper<SysDepart> queryWrapper = new QueryWrapper<>();
        queryWrapper.in("id", OrgunitIds);
        List<SysDepart> list = sysDepartMapper.selectList(queryWrapper);
...
//		this.save(user);  //保存角色的时候已经添加过一次了
    ...
    public Result<?> checkUserIsEffective(SysUser sysUser) {
        ...
        //情况1：根据用户信息查询，该用户不存在
        if (sysUser == null) {
            result.error500("该用户不存在，请联系系统管理员");
//            baseCommonService.addLog("用户登录失败，用户不存在！", CommonConstant.LOG_TYPE_1, null);
            return result;
        } else {
            String username = sysUser.getUsername();
            if (!username.equals("admin") && !username.equals("sysadmin") && !username.equals("secadmin") && !username.equals("secaudit")) {
...
//            baseCommonService.addLog("用户登录失败，用户名:" + sysUser.getUsername() + "已冻结！", CommonConstant.LOG_TYPE_1, null);
                        ...
                        result.error500("该用户待审核");
                        return result;
                    } else if (auditStatus == 3) {
                        result.error500("该用户新增未通过");
                        return result;
                    }
                } else {
                    result.error500("该用户审核状态为空，请校验");
                }
            }
        }
        return result;
    ...
    // ==================== 生产者：将请求放入队列 ====================
    @CacheEvict(value = {CacheConstant.SYS_USERS_CACHE}, allEntries = true)
    public void editUserNew(SysUser user, String roles, String departs) {
        if (user == null || user.getId() == null) {
            throw new IllegalArgumentException("用户ID不能为空");
        }
        // 1. 构建请求实体
        UserEditQueueConfig.UserEditRequest request = new UserEditQueueConfig.UserEditRequest();
        request.setUser(user);
        request.setRoles(roles);
        request.setDeparts(departs);

        // 2. 获取该用户的队列，放入请求（非阻塞，队列满则抛异常/降级）
        BlockingQueue<UserEditQueueConfig.UserEditRequest> userQueue = queueConfig.getUserQueue(user.getId());
        boolean offer = userQueue.offer(request);
        if (!offer) {
            throw new RuntimeException("用户编辑请求队列已满，请稍后重试");
        }

        // 3. 触发消费者处理（异步执行）
        processUserEditQueue(user.getId());
    }

    // ==================== 消费者：异步处理队列中的请求 ====================
    @Async("userEditExecutor") // 使用单线程执行器
    @Transactional(rollbackFor = Exception.class)
    public void processUserEditQueue(String userId) {
        BlockingQueue<UserEditQueueConfig.UserEditRequest> userQueue = queueConfig.getUserQueue(userId);
        UserEditQueueConfig.UserEditRequest request;
        // 循环消费队列（直到队列为空）
        while ((request = userQueue.poll()) != null) {
            try {
                // 执行原有的编辑逻辑
                doEditUser(request.getUser(), request.getRoles(), request.getDeparts());
            } catch (Exception e) {
                // 异常处理：记录日志、重试（可选）、告警
                log.error("处理用户{}编辑请求失败", userId, e);
                // 可选：重试机制（避免请求丢失）
                // if (retryCount < 3) {
                //     userQueue.offer(request, 1, TimeUnit.SECONDS);
                //     retryCount++;
                // }
            }
        }
    ...
    // ==================== 核心编辑逻辑（原有逻辑抽离） ====================
    private void doEditUser(SysUser user, String roles, String departs) {
        // step.1 修改用户基础信息
        this.updateById(user);
        ...
        // step.3 修改部门
        String[] arr = oConvertUtils.isNotEmpty(departs) ? departs.split(",") : new String[0];
        // 查询已关联部门
        List<SysUserDepart> userDepartList = sysUserDepartMapper.selectList(new QueryWrapper<SysUserDepart>().lambda().eq(SysUserDepart::getUserId, user.getId()));
     ...
     */
    @Override
    public LoginUser getEncodeUserInfo1(String username) {
        if (oConvertUtils.isEmpty(username)) {
            return null;
        }
        LoginUser loginUser = new LoginUser();
        SysUser sysUser = userMapper.getUserByRealName(username);
        if (sysUser == null) {
            return null;
        }
        BeanUtils.copyProperties(sysUser, loginUser);
