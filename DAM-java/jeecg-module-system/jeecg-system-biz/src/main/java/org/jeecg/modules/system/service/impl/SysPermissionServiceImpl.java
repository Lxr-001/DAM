...
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements ISysPermissionService {
     ...
     * 将权限ID列表转换为权限名称列表
     *
     * @param permissionIds 需要转换的权限ID列表
     * @return 对应的权限名称列表（按输入ID顺序）
     */
    public List<String> convertIdsToNames(List<String> permissionIds) {
        ...
        // 2. 构建ID->权限对象的映射表
        Map<String, SysPermission> permissionMap = allPermissions.stream()
                .collect(Collectors.toMap(
                        SysPermission::getId,
                        permission -> permission,
                        (existing, replacement) -> existing // 处理重复键（保留先出现的）
                ));

        // 3. 转换ID列表为名称列表
        return permissionIds.stream()
                .map(id -> permissionMap.get(id))      // 查找对应的权限对象
                .filter(Objects::nonNull)              // 过滤掉不存在的ID
                .map(SysPermission::getName)           // 提取名称
                .collect(Collectors.toList());         // 收集为列表
    }

    /**
     * 增强版 - 保留原始ID顺序，未找到的ID使用占位符
     *
     * @param placeholder 未找到ID时使用的占位符（null表示跳过）
     */
    public List<String> convertIdsToNamesWithPlaceholder(
        ...
        Map<String, String> nameMap = allPermissions.stream()
                .collect(Collectors.toMap(
                        SysPermission::getId,
        ...
        return permissionIds.stream()
                ...
                .filter(Objects::nonNull) // 如果placeholder为null，则过滤掉未找到的项
     ...
     * 适用于权限表数据量大的情况
     */
    public List<String> convertIdsToNamesBatch(List<String> permissionIds) {
        if (permissionIds == null || permissionIds.isEmpty()) {
            return Collections.emptyList();
        }

        // 1. 获取去重的ID集合
        Set<String> uniqueIds = new HashSet<>(permissionIds);

        // 2. 分批查询权限数据
        List<SysPermission> foundPermissions = new ArrayList<>();
        int batchSize = 500;
        ...
        for (int i = 0; i < idList.size(); i += batchSize) {
            ...
            if (!batchIds.isEmpty()) {
                foundPermissions.addAll(sysPermissionMapper.selectBatchIds(batchIds));
            }
        }

        // 3. 构建ID->名称的映射
        Map<String, String> nameMap = foundPermissions.stream()
                .collect(Collectors.toMap(
                        SysPermission::getId,
                        SysPermission::getName
                ));
