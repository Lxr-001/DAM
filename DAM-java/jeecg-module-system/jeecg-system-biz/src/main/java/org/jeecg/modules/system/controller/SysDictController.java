...
public class SysDictController {
    ...
    public Result<List<DictModel>> getDictItems(@PathVariable("dictCode") String dictCode, @RequestParam(value = "sign", required = false) String sign, HttpServletRequest request) {
        ...
        try {
            ...
            if ("system_security".equals(dictCode)) {
                LoginUser currentUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
                Integer userSecurity = currentUser.getSecurity();
                int security;
                if (userSecurity != null && systemSecurity != null) {
                    security = Math.min(userSecurity, systemSecurity);
                } else {
                    security = 0;
                }

                ls = ls.stream().filter(x -> Integer.parseInt(x.getValue()) <= security).collect(Collectors.toList());
            }
            if (ls == null) {
