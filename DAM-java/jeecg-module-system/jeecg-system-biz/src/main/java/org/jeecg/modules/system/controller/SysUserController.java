...
public class SysUserController {
    ...
    @Value("${k0.syOpen}")
    private boolean syOpen;
    @Value("${k0.syNames}")
    private String syNames;
    ...
    private void addLog(String logContent) {
        ...
        baseCommonService.addLog(Math.min(systemSecurity, loginUser.getSecurity()), sysUserService.getLogTypeByUserName(loginUser.getUsername()), 8,//用户/角色变更
                "用户列表", logContent, "成功");
    }
     ...
     * @return
     */
    @ApiOperation(value = "分页列表查询用户 特定密级下", notes = "试分页列表查询用户 特定密级下")
    @GetMapping(value = "/listInSecurity")
    public Result<IPage<SysUser>> listInSecurity(SysUser sysUser, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, @RequestParam(name = "maxDataSecurity", defaultValue = "1") Integer maxDataSecurity, HttpServletRequest req) {
        ...
        queryWrapper.ge("security", maxDataSecurity);
        queryWrapper.ne("id", userId);
        queryWrapper.ne("username", "sysadmin");
        queryWrapper.ne("username", "secadmin");
        queryWrapper.ne("username", "secaudit");
    ...
    public Result<IPage<SysUser>> queryPageList(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        ...
        if (oConvertUtils.isNotEmpty(departId)) {
            ...
            query.ne(SysUserDepart::getUserId, userId);
        ...
        if (syOpen) {
            ...
            if ("sysadmin".equals(userName)) {
                queryWrapper.eq("username", "sysadmin");
                queryWrapper.eq("username", "secadmin");
                queryWrapper.eq("username", "secaudit");
            } else if ("secadmin".equals(userName)) {
                queryWrapper.eq("username", "sysadmin");
                queryWrapper.eq("username", "secadmin");
            } else if ("secaudit".equals(userName)) {
                queryWrapper.eq("username", "secaudit");
            }
        }
        ...
        List<String> orgunitIds = pageList.getRecords().stream().map(SysUser::getOrgunit).collect(Collectors.toList());
        if (orgunitIds != null && orgunitIds.size() > 0) {
            Map<String, String> useDepNames = sysUserService.getOrgunitNamesByOrgunitIds(orgunitIds);
            pageList.getRecords().forEach(item -> {
                item.setOrgCodeTxt(useDepNames.get(item.getOrgunit()));
            });
        }
     ...
     */
    @PermissionData(pageComponent = "system/UserList")
    @RequestMapping(value = "/listsy", method = RequestMethod.GET)
    public Result<IPage<SysUser>> queryPageList1(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        String loginUserName = "";
        this.addLog("查询用户列表");
        Result<IPage<SysUser>> result = new Result<>();
        QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(user, req.getParameterMap());
        ...
        if (orgunitIds != null && orgunitIds.size() > 0) {
            ...
            String finalLoginUserName = loginUserName;
            pageList.getRecords().forEach(item -> {
                item.setOrgunit(useDepNames.get(item.getOrgunit()));
            });
        }
    ...
    public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            //获取部门和单位名称并set
//            String departName = "";
//            String departId = jsonObject.getString("orgunit");
//            List<SysDepart> sysDepartCache = sysDepartMapper.selectList(Wrappers.emptyWrapper());

//            while (departId.length() > 2) {
//                String dId = departId;
//                SysDepart sd = sysDepartCache.stream().filter(s -> dId.equals(s.getId())).collect(Collectors.toList()).get(0);
//                if (departName.equals("")) {
//                    departName = sd.getDepartName();
//                } else {
//                    departName = sd.getDepartName() + "→" + departName;
//                }
//                departId = sd.getParentId();
//            }
            // user.setOrgunit(departName);

            String salt = oConvertUtils.randomGen(8);
            user.setSalt(salt);
            String passwordEncode = PasswordUtil.encrypt(user.getUsername(), user.getPassword(), salt);
            user.setPassword(passwordEncode);
            user.setStatus(1);//初建用户设置状态为禁用，只能由安全保密员开通登录
            user.setDelFlag(CommonConstant.DEL_FLAG_0);
            //用户表字段org_code不能在这里设置他的值
            user.setOrgCode(null);
//            用户新增编辑删除需要授权管理员审批，状态为审批中的用户不能登录
            user.setAuditStatus(1);
            // 保存用户走一个service 保证事务
            sysUserService.saveUser(user, selectedRoles, selectedDeparts);
            // baseCommonService.addLog("添加用户，username： " + user.getUsername()+",用户密级为", CommonConstant.LOG_TYPE_2, 2);
            result.success("新增用户申请已经提交到授权管理员！");
            String json = new ObjectMapper().setSerializationInclusion(JsonInclude.Include.NON_NULL).writeValueAsString(user);
            String content1 = "新增用户，名称：" + user.getRealname() + "，用户密级为：" + this.personSecurityText(user.getSecurity()) + "，完整信息：" + json;
            this.addLog(content1);
            LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
            //待办的枚举类型已经有：用户新增，用户编辑，用户删除，构建可读内容 加入到 待办事项的内容里
// pid, 类型，内容，状态（待审核：1），申请人：当前登录用户，审核人
//            通过枚举获得密级对应文本
            String securityName = "";
            try {
                Enums.secretLevelEnum securityEnum = Enums.secretLevelEnum.getByValue(user.getSecurity());
            ...
            String content = "用户名：" + user.getUsername() + "；姓名：" + user.getRealname() + ";密级：" + securityName + ";集团八位码：" + user.getWorkNo();
            sysTodoService.save(new SysTodo(userId, 1, content, 1, sysUser.getUsername(), "授权管理员"));
        } catch (Exception e) {
    ...
    public Result<SysUser> edit(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            if (sysUser == null) {
                ...
                SysUser user = JSON.parseObject(jsonObject.toJSONString(), SysUser.class);
                String newName = sysUser.getUsername();
                QueryWrapper<SysTodo> queryWrapper = new QueryWrapper<>();
                queryWrapper.in("type", 2, 3);
                queryWrapper.eq("status", 1);
                queryWrapper.like(StringUtils.isNotBlank(newName), "content", "用户名：" + newName);

                List<SysTodo> std = sysTodoService.list(queryWrapper);
                if (std.size() > 0) {
                    return Result.error("该用户存在未审批操作！");
                }
...
//                user, roles, departs 放进 SysTodo 的 remark, 授权管理员通过后在从 remark 中读取出来再继续执行  sysUserService.editUser(user, roles, departs); 这个事务
                result.success("修改用户申请已经提交到授权管理员!");

                String orgunit = user.getOrgunit();
                SysDepart depart = sysDepartService.getById(orgunit);
                String oriOrgUnit = sysUser.getOrgunit();
                SysDepart depart1 = sysDepartService.getById(oriOrgUnit);
                String departNameOld = "";
                if (depart1 != null) {
                    departNameOld = depart1.getDepartName();
                }
                String usernameOld = "";
                String realnameOld = "";
                Integer securityOld = 0;
                if (sysUser != null) {
                    usernameOld = sysUser.getUsername();
                    realnameOld = sysUser.getRealname();
                    securityOld = sysUser.getSecurity();
                }
                String departName = "";
                if (depart != null) {
                    departName = depart.getDepartName();
                }
//                添加角色的变更，对于非改变项直接显示信息，只有改变项才显示为 -> 格式
                String content, auditContent;
                StringBuilder auditBuilder = new StringBuilder();

                // 用户名变更
                if (!usernameOld.equals(user.getUsername())) {
                    auditBuilder.append("用户名：").append(usernameOld).append("->").append(user.getUsername()).append("；");
                } else {
                    auditBuilder.append("用户名：").append(user.getUsername()).append("；");
                }

                // 姓名变更
                if (!realnameOld.equals(user.getRealname())) {
                    auditBuilder.append("姓名：").append(realnameOld).append("->").append(user.getRealname()).append("；");
                } else {
                    auditBuilder.append("姓名：").append(user.getRealname()).append("；");
                }

                // 密级变更
                if (!securityOld.equals(user.getSecurity())) {
                    auditBuilder.append("密级：").append(this.personSecurityText(securityOld)).append("->").append(this.personSecurityText(user.getSecurity())).append("；");
                } else {
                    auditBuilder.append("密级：").append(this.personSecurityText(user.getSecurity())).append("；");
                }

                // 部门变更
                if (!departNameOld.equals(departName)) {
                    auditBuilder.append("部门：").append(departNameOld).append("->").append(departName).append("；");
                } else {
                    auditBuilder.append("部门：").append(departName).append("；");
                }

                // 八位码（新增时无旧值，直接显示）
                auditBuilder.append("八位码：").append(user.getWorkNo()).append("；");

                // 角色变更
                String oldRoleNames = "";
                String newRoleNames = "";

                // 获取用户旧角色信息, 角色 id 在 roles 里，对应 sys_role 表的 id
                String[] roleIds = roles.split(",");
                String roleCode = sysUser.getRoleCode();
//                List<String> oldRoleIds = sysUserRoleService.getRole(sysUser.getUsername());
                List<SysRole> oldRoles = new ArrayList<>();
//                if (oldRoleIds != null && !oldRoleIds.isEmpty()) {
//                    oldRoles = sysRoleMapper.selectBatchIds(oldRoleIds);
//                }
                oldRoleNames = oldRoles.stream().map(SysRole::getRoleName).collect(Collectors.joining(","));

                // 获取用户新角色信息

                if (oConvertUtils.isNotEmpty(roles)) {
                    List<String> newRoleIdsList = Arrays.asList(roleIds);
                    List<SysRole> newRoles = sysRoleMapper.selectBatchIds(newRoleIdsList);
                    newRoleNames = newRoles.stream().map(SysRole::getRoleName).collect(Collectors.joining(","));
                    oldRoleNames = newRoleNames;
                }

                // 比较角色变更
                if (!oldRoleNames.equals(newRoleNames)) {
                    auditBuilder.append("角色：").append(oldRoleNames.isEmpty() ? "无" : oldRoleNames).append("->").append(newRoleNames.isEmpty() ? "无" : newRoleNames).append("；");
                } else {
                    auditBuilder.append("角色：").append(newRoleNames.isEmpty() ? "无" : newRoleNames).append("；");
                }

                auditContent = auditBuilder.toString();
//                        auditContent = UserComparator.compareUsers(sysUser, user);

                String json = new ObjectMapper().setSerializationInclusion(JsonInclude.Include.NON_NULL).writeValueAsString(user);
                switch (sysUserService.getLogTypeByUserName(((LoginUser) SecurityUtils.getSubject().getPrincipal()).getUsername())) {
                    case 2: // 系统参数管理日志
                        content = "编辑用户：" + auditContent;
                        break;
                    case 4: // 安全管理日志
                        content = "安全保密员修改 " + user.getRealname() + " 用户信息，用户密级为：" + this.personSecurityText(user.getSecurity()) + "，完整信息：" + json;
                        break;
                    default:
                        content = "编辑用户：" + auditContent;
                        break;
                ...
                this.addLog(content);
//                把 content 添加到 sysTodo, 状态设置为待审核，类型为用户编辑
                LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
                // pid, 类型，内容，remark 里保存json map string，状态（待审核：1），申请人：当前登录用户，审核人
                ObjectMapper mapper = new ObjectMapper();
                String jsonString = mapper.writeValueAsString(jsonObject);

                sysTodoService.save(new SysTodo(sysUser.getId(), 2, auditContent, jsonString, 1, loginUser.getUsername(), "授权管理员"));
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            result.error500("操作失败");
    ...
    @GetMapping(value = "/changeSecretLevel")
    public Result<SysUser> changeSecretLevel(String id, Integer level) {
        Result<SysUser> result = new Result<>();
        LambdaUpdateWrapper<SysUser> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(SysUser::getId, id);
//        updateWrapper.set(SysUser::getSecretLevel,level);
        sysUserService.update(updateWrapper);
        return result;
    }

    @RequestMapping(value = "/editBaseSetting", method = {RequestMethod.PUT, RequestMethod.POST})
    public Result<SysUser> editBaseSetting(@RequestBody JSONObject jsonObject) {
        Result<SysUser> result = new Result<>();
        try {
            LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
            ...
            if (sysUser == null) {
                ...
                String orgunitId = user.getOrgunit();
                SysDepart sysDepart = sysDepartMapper.selectById(orgunitId);
                user.setOrgunit(sysDepart.getDepartName());
                user.setPassword(sysUser.getPassword());
                String roles = jsonObject.getString("selectedroles");
                String departs = jsonObject.getString("selecteddeparts");
                if (oConvertUtils.isEmpty(departs)) {
    ...
    public Result<?> delete(@RequestParam(name = "id") String id) {
        ...
        this.addLog("申请删除用户，名称：" + user.getRealname());
        user.setAuditStatus(1);
        sysUserService.updateById(user);
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        String content = "用户名：" + user.getUsername() + "；姓名：" + user.getRealname() + ";工号：" + user.getWorkNo();
        sysTodoService.save(new SysTodo(id, 3, content, 1, sysUser.getUsername(), "授权管理员"));
        return Result.ok("删除用户申请成功");
    }
    ...
    public Result<?> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        if (idList.size() > 0) {
            nameList = sysUserService.list(new LambdaQueryWrapper<SysUser>().in(SysUser::getId, idList)).stream().map(SysUser::getRealname).collect(Collectors.toList());
        }
        this.addLog("批量删除用户，名称：" + String.join("、", nameList));
        ...
        try {
            ...
            for (String id : arr) {
                if (oConvertUtils.isNotEmpty(id)) {
                    this.sysUserService.update(new SysUser().setStatus(Integer.parseInt(status)), new UpdateWrapper<SysUser>().lambda().eq(SysUser::getId, id));
                }
            }
            ...
            if ("1".equalsIgnoreCase(status)) {
                this.addLog("启用用户，名称：" + String.join("、", nameList));
            } else if ("2".equalsIgnoreCase(status)) {
                this.addLog("禁用用户，名称：" + String.join("、", nameList));
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    @RequestMapping(value = "/queryNameByIds", method = RequestMethod.GET)
    public Result<String> queryNameByIds(@RequestParam(name = "userIdList") String userIdList) {
        Result<String> result = new Result<>();
        if (userIdList.isEmpty()) {
            result.error500("用户ID列表不能为空");
            return result;
        }
        ...
        if (userList == null || userList.isEmpty()) {
            ...
            // 使用 String.join() 方法将所有用户的 realname 拼接成一个字符串
            String realnames = userList.stream().map(SysUser::getRealname).collect(Collectors.joining(", "));
            result.setResult(realnames);
            result.setSuccess(true);
        }
        return result;
    }
    ...
    /**
     * 获取当前登录用户的 id -> 得到 当前用户部门 id(SysUser 表 orgunit 字段) ->
     * 在部门管理中（sys/sysDepart/queryTreeList）能查到父节点甚至 根节点的 id（部门数组）, 能获取到 角色 id,
     * 从 sys_user_role表（三个字段id, user_id, role_id, 表示具有 user_id 的用户具备 role_id 的具体角色，具体的角色表是 sys_role） 能获取到 所有的 关联用户，
     * 遍历这些用户 id 在 同部门数组里的形成一个 IPage<List<SysUser>>返回给前端
     *
     * @return
     */
    @GetMapping(value = "/getApproverList")
    public Result<IPage<List<SysUser>>> getApproverList(@RequestParam(name = "maxDataSecurity", defaultValue = "1") Integer maxDataSecurity) {
        Result<IPage<List<SysUser>>> result = new Result<>();
        try {
            // 1. 获取当前登录用户
            LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
            QueryWrapper<SysRole> roleRoleQueryWrapper = new QueryWrapper<>();
            roleRoleQueryWrapper.eq("role_code", "approver");
            List<SysRole> roleRoleList = sysRoleService.list(roleRoleQueryWrapper);
            String roleId = roleRoleList.get(0).getId();
            if (loginUser == null) {
                result.error500("未获取到当前登录用户信息");
                return result;
            }
            String currentUserId = loginUser.getId();

            // 2. 获取当前用户的部门ID（orgunit字段）
            SysUser currentUser = sysUserService.getById(currentUserId);
            if (currentUser == null) {
                result.error500("未找到当前用户的详细信息");
                return result;
            }

            // 3. 获取当前用户的部门ID及父级部门ID数组（从orgCode字段解析）
            List<String> departmentIds = new ArrayList<>();
            String orgCode = loginUser.getOrgCode();
            if (StringUtils.isNotEmpty(orgCode)) {
                departmentIds = Arrays.asList(orgCode.split(","));
            } else {
                // 如果orgCode为空，尝试从orgunit获取当前部门ID
                String currentDepartId = currentUser.getOrgunit();
                if (StringUtils.isNotEmpty(currentDepartId)) {
                    departmentIds.add(currentDepartId);
                    // 尝试获取父部门ID
                    SysDepart depart = sysDepartService.getById(currentDepartId);
                    if (depart != null) {
                        String parentId = depart.getParentId();
                        while (StringUtils.isNotEmpty(parentId) && !parentId.equals("0")) {
            ...
            if (departmentIds.isEmpty()) {
                result.error500("未找到当前用户的部门信息");
                return result;
            }

            // 4. 从sys_user_role表获取所有用户角色关系
            ...
            // 5. 遍历这些用户，筛选出在同部门数组里的用户
            List<SysUser> resultUserList = new ArrayList<>();
            Set<String> processedUserIds = new HashSet<>(); // 避免重复添加用户

            for (SysUserRole userRole : userRoles) {
                String userId = userRole.getUserId();

                // 跳过已处理的用户和当前登录用户
                if (processedUserIds.contains(userId) || userId.equals(currentUserId)) {
                    continue;
                }

                // 获取用户信息
                SysUser user = sysUserService.getById(userId);
                if (user != null && !StringHelper.isNullOrEmptyString(user.getOrgunit())) {
                    // 获取用户所在部门信息
                    String[] userDepartIds = user.getOrgunit().split(",");

                    // 检查用户是否在当前用户的部门数组中
                    boolean isInSameDepartment = false;
                    for (String userDepartId : userDepartIds) {
                        if (departmentIds.contains(userDepartId)) {
                    ...
                    // 如果在同一部门，添加到结果列表
                    if (isInSameDepartment && user.getSecurity() >= maxDataSecurity) {
            ...
            // 创建Page对象并包装结果用户列表
            Page<List<SysUser>> page = new Page<>();
            List<List<SysUser>> pageList = new ArrayList<>();
            pageList.add(resultUserList);
            page.setRecords(pageList);
            page.setTotal(resultUserList.size());
            page.setCurrent(1);
            page.setSize(resultUserList.size());
    ...
    public Result<?> changePassword(@RequestBody SysUser sysUser) {
        ...
        this.addLog("修改用户密码，用户名称：" + u.getRealname());
        sysUser.setId(u.getId());

        //检查新旧密码不能重复
        String newPasswordOldEncode = PasswordUtil.encrypt(u.getUsername(), sysUser.getPassword(), u.getSalt());
        if (newPasswordOldEncode.equals(u.getPassword())) {
            return Result.error("新密码不能与旧密码重复！");
        }
    ...
    public Result<?> resetPassword(String username) {
        ...
        this.addLog("重置用户密码，用户名称：" + u.getRealname());
//        u.setId(u.getId());
        u.setPassword("123456");
        u.setPasswdUpdateTime(null);
        //检查新旧密码不能重复
//        String newPasswordOldEncode = PasswordUtil.encrypt(u.getUsername(), sysUser.getPassword(), u.getSalt());
//        if (newPasswordOldEncode.equals(u.getPassword())) {
//            return Result.error("新密码不能与旧密码重复！");
//        }
        return sysUserService.resetPassword(u);
    }
    ...
    public Result<?> updatePassword(@RequestBody JSONObject json) {
        ...
        this.addLog("修改用户密码，名称：" + user.getRealname());
    ...
    public Result<String> addSysUserRole(@RequestBody SysUserRoleVO sysUserRoleVO) {
        ...
        try {
            String roleName = sysRoleService.getOne(new LambdaQueryWrapper<SysRole>().eq(SysRole::getId, sysUserRoleVO.getRoleId())).getRoleName();
            ...
            for (String sysUserId : sysUserRoleVO.getUserIdList()) {
                ...
                String realname = sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, sysUserId)).getRealname();
                String content = "修改用户角色，将用户 " + realname + " 添加到 " + roleName + " 角色";
                this.addLog(content);
            }
    ...
    @RequestMapping(value = "/addSysUserRoleTodo", method = RequestMethod.POST)
    public Result<String> addSysUserRoleTodo(@RequestBody SysUserRoleVO sysUserRoleVO) {
        Result<String> result = new Result<String>();
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        String re = "";
        try {
            ...
            for (String sysUserId : sysUserRoleVO.getUserIdList()) {
                ...
                if (one == null) {
                    //sysUserRoleService.save(sysUserRole);
                    String realname = sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, sysUserId)).getRealname();
                    QueryWrapper<SysTodo> queryWrapper1 = new QueryWrapper<>();
                    queryWrapper1.and(wrapper -> wrapper.eq("type", 9)).eq("status", 1)
                            .like(StringUtils.isNotBlank(roleName), "content", roleName)
                            .like(StringUtils.isNotBlank(realname), "content", realname);
                    List<SysTodo> std = sysTodoService.list(queryWrapper1);
                    if (std.size() > 0) {
                        if ("".equals(re)) {
                            re = realname;
                        } else {
                        ...
                        String content = "新增用户角色，将用户 " + realname + " 添加到 " + roleName + " 角色，已提交授权管理员审批";
                        this.addLog(content);
                        NameValuePair role_id = new NameValuePair("role_id", sysRoleId);
                        NameValuePair user_id = new NameValuePair("user_id", sysUserId);
                        NameValuePair real_name = new NameValuePair("real_name", realname);
                        NameValuePair role_Name = new NameValuePair("role_Name", roleName);
                        String remark = toJson(role_id, user_id, real_name, role_Name);
                        sysTodoService.save(new SysTodo(9, content, remark, 1, user.getUsername(), "授权管理员"));
                    }
                } else {
                    result.setMessage("已经存在该角色!");
                }
            }
            if ("".equals(re)) {
                result.setMessage("已提交授权管理员审批!");
            } else {
                result.setMessage("该角色下：" + re + " 存在待审批任务，无法重复提交，其他用户已提交！");
            }
            result.setSuccess(true);
            return result;
        } catch (Exception e) {
    ...
    public Result<SysUserRole> deleteUserRole(@RequestParam(name = "roleId") String roleId, @RequestParam(name = "userId", required = true) String userId) {
        ...
        try {
            ...
            String userName = sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, userId)).getRealname();
            String roleName = sysRoleService.getOne(new LambdaQueryWrapper<SysRole>().eq(SysRole::getId, roleId)).getRoleName();
            this.addLog("修改用户角色，删除用户 " + userName + " 的 " + roleName + " 角色");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    public Result<SysUserRole> deleteUserRoleBatch(@RequestParam(name = "roleId") String roleId, @RequestParam(name = "userIds", required = true) String userIds) {
        ...
        try {
            ...
            sysUserRoleService.remove(queryWrapper);
            result.success("删除成功!");
            List<String> idList = new ArrayList<>(Arrays.asList(userIds.split(",")));
            List<String> userNameList = sysUserService.list(new LambdaQueryWrapper<SysUser>().in(SysUser::getId, idList)).stream().map(SysUser::getRealname).collect(Collectors.toList());
            String roleName = sysRoleService.getOne(new LambdaQueryWrapper<SysRole>().eq(SysRole::getId, roleId)).getRoleName();
            this.addLog("修改用户角色，删除角色 " + roleName + " 下的 " + String.join("、", userNameList) + " 用户");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            result.error500("删除失败！");
        }
     ...
     */
    //@RequiresRoles({"admin"})
    @RequestMapping(value = "/deleteUserRoleBatchTodo", method = RequestMethod.DELETE)
    public Result<String> deleteUserRoleBatchTodo(@RequestParam(name = "roleId") String roleId, @RequestParam(name = "userIds", required = true) String userIds) {
        Result<String> result = new Result<String>();
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        try {
//            QueryWrapper<SysUserRole> queryWrapper = new QueryWrapper<SysUserRole>();
...
//            sysUserRoleService.remove(queryWrapper);
            ...
            for (String s : userNameList) {
                QueryWrapper<SysTodo> queryWrapper1 = new QueryWrapper<>();
                queryWrapper1.and(wrapper -> wrapper.eq("type", 10)).eq("status", 1)
                        .like(StringUtils.isNotBlank(roleName), "content", roleName)
                        .like(StringUtils.isNotBlank(s), "content", s);
                List<SysTodo> std = sysTodoService.list(queryWrapper1);
                if (std.size() > 0) {
                    if ("".equals(content1)) {
                        content1 = s;
                    ...
                    newUserNameList.add(s);
                }
            }
            if (newUserNameList.size() > 0) {
                String content = "修改用户角色，删除角色 " + roleName + " 下的 " + String.join("、", newUserNameList) + " 用户,提交管理员审批";
                this.addLog(content);
                NameValuePair role_id = new NameValuePair("role_id", roleId);
                NameValuePair user_Ids = new NameValuePair("user_Ids", userIds);
                NameValuePair role_Name = new NameValuePair("role_Name", roleName);
                NameValuePair user_Names = new NameValuePair("user_Names", userNameList);
                String remark = toJson(role_id, user_Ids, role_Name, user_Names);
                sysTodoService.save(new SysTodo(10, content, remark, 1, user.getUsername(), "授权管理员"));
            }
            if (!"".equals(content1)) {
                result.setMessage("该角色下：" + content1 + " 存在待审批任务，无法重复提交，其他用户已提交！");
            } else {
                result.setMessage("已提交授权管理员审批!");
            }
            result.setSuccess(true);
...
//	}
    ...
    public Result putRecycleBin(@RequestBody JSONObject jsonObject, HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(userIds)) {
            ...
            List<String> idList = new ArrayList<>(Arrays.asList(userIds.split(",")));
            List<String> userNameList = sysUserService.list(new LambdaQueryWrapper<SysUser>().in(SysUser::getId, idList)).stream().map(SysUser::getRealname).collect(Collectors.toList());
            this.addLog("还原回收站中的用户：" + String.join("、", userNameList));
        }
    ...
    public Result deleteRecycleBin(@RequestParam("userIds") String userIds) {
        if (StringUtils.isNotBlank(userIds)) {
            SysUser updateUser = new SysUser();
            updateUser.setUpdateBy("a");
            updateUser.setUpdateTime(new Date());
            sysUserService.revertLogicDeleted(Arrays.asList(userIds.split(",")), updateUser);
            //  （局管）只能先还原再删除，底层Mapper被改了，del_flage=1的记录查不到

            List<String> idList = new ArrayList<>(Arrays.asList(userIds.split(",")));
            List<String> userNameList = sysUserService.list(new QueryWrapper<SysUser>().in("id", idList)).stream().map(SysUser::getRealname).collect(Collectors.toList());
            this.addLog("彻底删除回收站中的用户：" + String.join("、", userNameList));
    ...
    public Result<List<SysUser>> appQueryUser(@RequestParam(name = "keyword", required = false) String keyword, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
        Result<List<SysUser>> result = new Result<List<SysUser>>();
